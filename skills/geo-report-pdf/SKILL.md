...
- The PDF generation script is located at: `"${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/geo}/scripts/generate_pdf_report.py"`
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/skills/geo}/scripts/generate_pdf_report.py" /tmp/geo-audit-data.json GEO-REPORT-<DOMAIN-SLUG>.pdf
```
