...
- **Retired-skill pruning in `install.sh` / `install-win.sh`** — the install copy loop is additive, so a skill removed from the repo survived forever in `~/.claude/skills`, advertising a command whose backing script no longer exists. Upgrades now remove `geo-report-pdf` and `geo-proposal` by name; user-only skills are never touched. Verified against a real stale install: both directories pruned, `geo-observe` preserved.
